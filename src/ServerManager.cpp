...
void ServerManager::printServers() const {
  ...
  for (size_t i = 0; i < _servers.size(); i++) {
    ...
    std::cout << "AutoIndex: " << _servers[i]->isAutoIndexEnabled()
              << std::endl;
    std::cout << "Is default server: " << _servers[i]->isDefaultServer()
              << std::endl;
    std::cout << "MaxBodySize: " << _servers[i]->getMaxBodySize() << std::endl;
    std::cout << "Root: " << _servers[i]->getRoot() << std::endl;

    std::cout << "Redirection: " << _servers[i]->getRedirection().first << " "
              << _servers[i]->getRedirection().second << std::endl;
...
void ServerManager::printLocations(std::vector<Location> locations) const {
  for (std::vector<Location>::iterator it = locations.begin();
    ...
    std::cout << "	Redirection: " << it->redirection.first << " "
    ...
    for (std::vector<LimitExceptDirective>::iterator led =
      ...
      std::cout << " Allow: " << led->allow << " Deny: " << led->deny
                << std::endl;
    }
 ...
 * response. It also checks if the request is a CGI request. If the request is
...
void ServerManager::handleClientRequest(int clientFD) {
  ...
  buffer[rBytes] = '\0';
  Logger::log(INFO, "[ServerManager] Received from client FD " +
  ...
  if (isRequestComplete(_clientBuffers[clientFD])) {
    ...
    try {
      ...
      // Logger::log(INFO, "[ServerManager] Response for client FD " +
      // std::to_string(clientFD) + " ResMsg: \n" + response.getMsg());
...
void ServerManager::removeClient(int clientFD) {
  ...
  Logger::log(INFO, "[ServerManager] Client FD " + std::to_string(clientFD) +
                        " disconnected.");
}
 ...
 * for the rest check if the Content-Length header is present and if the body is
 * complete
 */
...
void ServerManager::closeFDs() {
  ...
  for (std::map<int, Server *>::iterator it = _clientToServer.begin();
    ...
    close(clientFD);
    Logger::log(INFO,
                "[ServerManager] Closed client FD " + std::to_string(clientFD));
  }
  ...
  for (std::vector<std::unique_ptr<Server>>::iterator it = _servers.begin();
    ...
    close(serverFD);
    Logger::log(INFO,
                "[ServerManager] Closed server FD " + std::to_string(serverFD));
  }
