...
void Server::setConfiguration(const ServerConfig &config)
{
  this->setHost(config.host);
  this->setPort(config.port);
  this->setAutoIndex(config.autoIndex);
  this->setDefaultServer(config.defaultServer);
  this->setMaxBodySize(config.maxBodySize);
  this->setRoot(config.root);
  this->setServerName(config.serverName);
  this->setRedirection(config.redirection);
  this->setErrorPages(config.errorPages);
  this->setIndex(config.index);
  this->setLocations(config.locations);
}
